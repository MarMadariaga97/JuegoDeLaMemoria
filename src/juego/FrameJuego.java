...
public class FrameJuego {
	...
	public FrameJuego(Tablero t) {
		...
		this.cartas=new JButton[t.getDimension()][t.getDimension()];
		
		cargarCartas(panelPrincipal);
	...
	public void cargarCartas(JPanel p) {
		
	for (int i = 0; i < t.getDimension(); i++) {
		for (int j = 0; j < t.getDimension(); j++) {
			JButton carta = new JButton("b" + i +j);
			this.cartas[i][j] = carta;
			carta.setFont(carta.getFont().deriveFont(Font.BOLD, 30f)); // Creo una fuente de tamaño 24
			carta.setHorizontalAlignment(SwingConstants.CENTER); // Centrar el texto
